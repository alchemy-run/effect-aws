...
import { StateStore, type StateStoreError, type StateStore as StateStoreInterface } from "./state-store.ts";
import type { ChannelType, Conversation, ThreadInfo } from "./thread.ts";

/**
 * Represents the organization configuration defined in code.
 * This includes all agents, channels, and groups that are part of the organization.
 */
...
export interface OrgService extends StateStoreInterface {
   ...
   */
  readonly listChannels: () => Effect.Effect<readonly Channel[], StateStoreError>;
...
export const createOrg = (options: CreateOrgOptions) =>
  Effect.gen(function* () {
    ...
    const service: OrgService = {
      ...
      // Agent methods
      getAgent: (agentId) =>
        Effect.succeed(agentMap.get(agentId)),

      listAgents: () =>
        Effect.succeed(config.agents),

      // Channel methods
      getChannel: (channelId) =>
        Effect.succeed(channelMap.get(channelId)),

      listChannels: () =>
        Effect.succeed(config.channels),

      // Group methods
      getGroup: (groupId) =>
        Effect.succeed(groupMap.get(groupId)),

      listGroups: () =>
        Effect.succeed(config.groups),
      ...
      createReplyThread: (parentMessageId, channelType, channelId, participants) =>
        Effect.gen(function* () {
          const now = Date.now();
